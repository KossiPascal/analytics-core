...
import { TODAY_YEAR_MONTH_DAY, getYearsList, getMonthsList, currentMonth, initTabsLinkView } from '@kossi-shared/functions';
...
export class ManagementsComponent implements OnInit {
  ...
  async START_CALCULATION(event: Event) {
    ...
    if (calcul === true) {
      return this.snackbar.show({ msg: 'Calcul effectué avec succès', color: 'success', duration: 5000 });
    }
    return this.snackbar.show({ msg: 'Erreur lors du calcul des indicateurs, ', color: 'danger', duration: 5000 });
  }
