...
export class PcimeComponent {
  ...
  sendDataToDhis2(dhis2FormGroup: FormGroup) {
    ...
    if (this.PCIMNE_NEWBORN$ && dhis2FormGroup) {
      ...
      this.PCIMNE_NEWBORN$.orgunit = orgunit_uid;
      ...
      const dhis2Params:Dhis2DataValueSetParams = {
        ...
        data: this.PCIMNE_NEWBORN$,
        period: period,
      }

        this.snackbar.show(`Envoi des données du au DHIS2 sur ${orgunit_name}`, { backgroundColor: 'success', position: 'TOP', duration:10000 });

        this.api.SendPcimneNewbornActivitiesToDhis2(dhis2Params).subscribe(async (_c$: { status: number, data: string }) => {
          if (_c$.status == 200) {
