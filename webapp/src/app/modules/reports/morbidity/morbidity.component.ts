...
import { IndicatorsDataOutput, MorbidityReport } from '@kossi-models/reports';
...
export class MorbidityComponent {
  ...
  REPPORTS_HEADER: ReportsHealth = {
    ...
    REPPORT_SUBTITLE: 'MORBIDITE 5 ANS ET PLUS',
  ...
  validateData() {
    ...
    this.api.ValidateMorbidityReports(this._formGroup.value).subscribe(async (_c$: { status: number, data: string }) => {
      if (_c$.status == 200) {
  ...
  cancelValidation(){
    ...
    this.api.CancelValidateMorbidityReports(this._formGroup.value).subscribe(async (_c$: { status: number, data: string }) => {
      if (_c$.status == 200) {
        this.SHOW_DATA(this._formGroup);
  ...
  SHOW_DATA(updatedFormGroup: FormGroup) {
    ...
    this.ldbfetch.GetMorbidityReports(this._formGroup.value, this.isOnline).then((_res$: IndicatorsDataOutput<MorbidityReport> | undefined) => {
  ...
  sendDataToDhis2(dhis2FormGroup: FormGroup) {
    ...
    if (this.MORBIDITY$ && dhis2FormGroup) {
      ...
      this.MORBIDITY$.orgunit = orgunit_uid;
      ...
      const dhis2Params:Dhis2DataValueSetParams = {
        ...
        this.api.SendMorbidityActivitiesToDhis2(dhis2Params).subscribe(async (_c$: { status: number, data: string }) => {
          if (_c$.status == 200) {
