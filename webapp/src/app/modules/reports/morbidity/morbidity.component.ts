...
export class MorbidityComponent {
  ...
  SHOW_DATA(updatedFormGroup: any) {
    ...
    this.ldbfetch.GetMorbidityReports(this._formGroup.value).then((_res$: IndicatorsDataOutput<MorbidityReport> | undefined) => {
  ...
  sendDataToDhis2() {
    ...
    if (this.MORBIDITY$) {
      ...
      const dataValues = this.dataTransformToDhis2(this.MORBIDITY$, period);

      // this.api.SendMorbidityActivitiesToDhis2({...this._formGroup.value, dataValues}).subscribe(async (_c$: { status: number, data: string }) => {
      //   if (_c$.status == 200) {
      //     this.SHOW_DATA(this._formGroup);
  ...
  dataTransformToDhis2(data: MorbidityReport, dataPeriod?: { year: number, month: string }): { dataValues: DataValue[] } {
