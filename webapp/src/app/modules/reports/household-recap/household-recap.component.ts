...
import { currentYear, currentMonth, getMonthsList, getYearsList, monthByArg, notNull, toArray } from '@kossi-src/app/utils/functions';
...
export class HouseholdRecapComponent implements OnInit, AfterViewInit {
  ...
  validateData(event: Event) {
    ...
    this.api.ValidateHouseholdRecapReports(this._formGroup.value).subscribe(async (_c$: { status: number, data: string }) => {
      if (_c$.status == 200) {
  ...
  SHOW_DATA(event: Event) {
    ...
    this.ldbfetch.GetHouseholdRecapReports(this._formGroup.value).then((_res$: {total:HouseholdRecapReport, out:IndicatorsDataOutput<HouseholdRecapReport[]>} | undefined) => {
        this.REGION_NAME = _res$?.out.region.name??'';
        this.RECO_ASC_TYPE = _res$?.out.reco_asc_type??'';
        this.RECO_ASC_NAME = (_res$?.out.reco_asc_type === 'RECO' ? (_res$?.out.reco?.name ?? '') : _res$?.out.chw.name) ?? '';
        this.PREFECTURE_NAME = _res$?.out.prefecture.name ?? '';
        this.COMMUNE_NAME = _res$?.out.commune.name ?? '';
        this.DISTRICT_QUARTIER_NAME = _res$?.out.district_quartier.name ?? '';
        this.VILLAGE_SECTEUR_NAME = _res$?.out.village_secteur.name ?? '';
        this.MONTH = monthByArg(this._formGroup.value.month).labelFR;
        this.YEAR = this._formGroup.value.year;
        this.IS_VALIDATE = _res$?.out.is_validate === true,
