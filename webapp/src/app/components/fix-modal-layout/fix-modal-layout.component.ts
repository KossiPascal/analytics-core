...
export class FixModalLayoutComponent {
  ...
  @Input() hideOnConfirm!: boolean;
  @Input() modalActionColor?: ModalColor['color'];
  ...
  this.fix.getModalVisibility().subscribe((visibility: boolean | undefined) => {
    ...
    this.fix.getHideOnConfirm().subscribe((res: boolean | undefined) => {
      this.hideOnConfirm = !!res;
    });
    this.fix.getModalActionColor().subscribe((res: ModalColor['color'] | undefined) => {
 ...
 async cancel() {
  ...
  async submit() {
    if (this.onSubmit) {
      ...
      try {
        await this.onSubmit();
        if (this.hideOnConfirm) this.visible = false;
