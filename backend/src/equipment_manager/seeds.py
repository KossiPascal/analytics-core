...
Usage CLI :
    ...
    flask em seed --module departments
...
def seed_departments():
    ...
    for code, name, parent_code in DEPARTMENTS_DATA:
        parent = dept_map.get(parent_code) if parent_code else None
        obj, created = _upsert(
            Department,
            {"code": code},
            {"name": name, "parent_id": parent.id if parent else None, "is_active": True},
        )
        dept_map[code] = obj
        _flush()
        if created:
    ...
    _commit()
    logger.info(f"[seed_departments] {created_total} enregistrements créés")
    return created_total
...
def seed_positions():
    from backend.src.equipment_manager.models.employees import Department, Position

    # Charger les départements existants
    dept_map = {d.code: d for d in Department.query.all()}
    created_total = 0
...
def seed_categories():
    from backend.src.equipment_manager.models.equipment import EquipmentCategoryGroup, EquipmentCategory
...
def seed_brands():
    from backend.src.equipment_manager.models.equipment import EquipmentBrand
...
def seed_problem_types():
    from backend.src.equipment_manager.models.tickets import ProblemType
...
def seed_employees():
    from backend.src.equipment_manager.models.employees import Position, Employee, EmployeeProfile
    from backend.src.databases.extensions import db
...
def seed_equipment():
    from backend.src.equipment_manager.models.equipment import Equipment, EquipmentCategory, EquipmentBrand
...
def seed_tickets():
    from backend.src.equipment_manager.models.tickets import RepairTicket, Issue, TicketEvent, ProblemType
