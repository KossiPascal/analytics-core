                ...
                SELECT jsonb_agg(
                    jsonb_build_object(
                        ...
                        'data', (
                                ...
                                WHERE v.family_id = f.id AND v.child_id IS NOT NULL AND v.reco_id = a.reco_id AND v.has_all_vaccine_done IS TRUE
                            )
                    ) ORDER BY f.given_name
                )
                FROM children_families f 
                WHERE f.reco_id = a.reco_id AND f.has_all_vaccine_done IS TRUE
