...
class QueryValidator:
        ...
        for item in select:
            ...
            if isinstance(item, dict):
                ...
                if field not in self.dimensions and field not in self.metrics:
                    raise QueryValidationError(
                        f"Unauthorized select field: {field}"
                    )
                ...
                if not alias:
                    raise QueryValidationError(
                        "Aggregated select requires an alias"
                    )
            ...
            # ===== Ancien format string =====
                ...
                if item not in self.dimensions and item not in self.metrics:
                    raise QueryValidationError(
                        f"Unauthorized select field: {item}"
                    )
    ...
    def validate_order_by(self):
        ...
        for o in order_by:
            ...
            # champ réel OU alias
            if (
                field not in self.dimensions
                and field not in self.metrics
                and field not in valid_aliases
            ):
                raise QueryValidationError(
                    f"Invalid order_by field or alias: {field}"
                )
        ...
        for item in select:
            if isinstance(item, dict):
                ...
                if item in self.metrics and item not in aggs:
                    raise QueryValidationError(
                        f"Metric '{item}' must be aggregated"
                    )
    ...
    def _is_select_object(self, item):
    ...
    def _validate_alias(self, alias: str, seen: Set[str]):
        ...
        if alias in self.dimensions or alias in self.metrics:
            raise QueryValidationError(
                f"Alias conflicts with existing field: {alias}"
            )
