...
from backend.src.okr_manager.models.models import UserRole, UserPermission
...
bp = Blueprint(f"okrmanager_{_cible}", __name__, url_prefix=f"/api/okrmanager/{_cible}")
...
def list_roles():
    try:
        roles: List[UserRole] = UserRole.query.filter_by(deleted=False).all()
...
def update_role(role_id: int):
    try:
        ...
        if "permission_ids" in data:
            perms:List[UserPermission] = UserPermission.query.filter(
                UserPermission.id.in_(data["permission_ids"]), 
