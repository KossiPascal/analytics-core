...
def list_levels():
    try:
        ...
        return error_response("Impossible de lister les niveaux", 500, str(e))
...
def get_level(level_id: int):
    try:
        ...
        if not lv or lv.deleted:
            return error_response(f"Niveau id={level_id} introuvable", 404)
        return jsonify(lv.to_dict()), 200
    except Exception as e:
        logger.error(f"Get level error: {e}")
        return error_response("Impossible de récupérer le niveau", 500, str(e))
...
def create_level():
    try:
        ...
        if level_num is None:
            return error_response("Le numéro de niveau est requis", 400)
        if not tenant_id:
            return error_response("tenant_id est requis", 400)
        ...
        db.session.rollback()
        return error_response("Ce numéro de niveau existe déjà pour ce tenant", 409)
    except Exception as e:
        db.session.rollback()
        logger.error(f"Create level error: {e}")
        return error_response("Impossible de créer le niveau", 500, str(e))
...
def update_level(level_id: int):
    try:
        ...
        db.session.rollback()
        return error_response("Impossible de mettre à jour le niveau", 500, str(e))
...
def delete_level(level_id: int):
    try:
        ...
        db.session.rollback()
        return error_response("Impossible de supprimer le niveau", 500, str(e))
