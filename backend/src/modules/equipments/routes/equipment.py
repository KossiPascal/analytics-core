...
from backend.src.modules.equipments.models.tickets import RepairTicket, TicketEvent
from backend.src.modules.templates.docs_generetor.pdf_generator import IMG_DIR, pdf_response
...
def generate_reception_pdf(id):
    ...
    context = {
        ...
        "employee_position": (
            employee.position.name if employee and employee.position else "—"
        ),
        "employee_district": district,
        "employee_zone": zone,
        "employee_phone": employee.phone if employee else "—",
        "employee_role_label": (
            employee.position.name if employee and employee.position else "ASC"
        ),
        # Équipement
        "equipment_type": (
