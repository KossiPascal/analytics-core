...
class Meeting(db.Model, BaseModel, TenantMixin, TimestampMixin, SoftDeleteMixin, AuditMixin, StatusMixin):
    ...
    transcriptions = db.relationship("MeetingTranscription", backref="meeting", lazy=True, cascade="all, delete-orphan")
    summaries = db.relationship("MeetingSummary", backref="meeting", lazy=True, cascade="all, delete-orphan")
...
class MeetingTranscription(db.Model, BaseModel, TenantMixin, TimestampMixin, SoftDeleteMixin, AuditMixin, StatusMixin):
    __tablename__ = "meeting_transcriptions"
    __table_args__ = {"schema": "meet"}

    meeting_id          = db.Column(db.String(11), db.ForeignKey("meet.meetings.id"), nullable=False)
...
class MeetingSummary(db.Model, BaseModel, TenantMixin, TimestampMixin, SoftDeleteMixin, AuditMixin, StatusMixin):
