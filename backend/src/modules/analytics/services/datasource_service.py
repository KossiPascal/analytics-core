...
class DataSourceProvisioningService:
    ...
    def list_full_datasources(tenant_id: int = None) -> list[DataSource]:
        query = DataSource.query.options(
            selectinload(DataSource.datasource_connection)
                .selectinload(DataSourceConnection.datasource_ssh_config),
            selectinload(DataSource.datasource_credential),
            selectinload(DataSource.datasource_permissions),
        )
    ...
    def update_full_datasource(
        ...
        *,
        ...
        try:
            ...
            if ssh_enabled is not None:
                ssh:DataSourceSSHConfig = ds.datasource_ssh_config
            ...
            cred.datasource_connection_id=conn.id,
            ...
            if permissions is not None:
                ...
                existing_perms = {p.user_id: p for p in ds.datasource_permissions}
                ...
                for perm_data in permissions:
                    ...
                    if user_id in existing_perms:
                        ...
                        if role is not None:
                            perm.datasource_connection_id=conn.id
                        if role is not None:
            ...
            # Audit
            audit = DataSourceHistory(
                datasource_connection_id=conn.id,
                datasource_id=ds.id,
                tenant_id=tenant_id,
                type=type,
                action="UPDATE",
    ...
    def delete_full_datasource(datasource_id: int, deleted_by: str = None):
        try:
            ...
            # Audit before delete
            audit = DataSourceHistory(
                datasource_connection_id=conn.id if conn else None,
                datasource_id=ds.id,
        ...
        try:
            ...
            if ssh_host or use_ssh_key or ssh_enabled:
                ssh_config = DataSourceSSHConfig.query.filter(
                    DataSourceSSHConfig.datasource_connection_id==connection.id
            ...
            if connection and (username or password or ssh_username):
                credential = DataSourceCredential.query.filter(
                    ...
                    DataSourceCredential.datasource_connection_id==connection.id
                ).first()
                ...
                if ssh_config and ssh_config.id:
                    credential.datasource_ssh_config_id = ssh_config.id
    ...
    def get_ssh_config(conn:DataSourceConnection, status: ConnectionStatus = ConnectionStatus.PROD):
        ...
        if not conn:
            return None
        return conn.datasource_ssh_config if conn.datasource_ssh_config and conn.datasource_ssh_config.is_active else None
    ...
    def get_credential(conn:DataSourceConnection, status: ConnectionStatus = ConnectionStatus.PROD):
        ...
        if not source:
            return None
        credential: DataSourceCredential = source.datasource_credential
        return credential if credential.datasource_connection_id == conn.id and credential.is_active else None
