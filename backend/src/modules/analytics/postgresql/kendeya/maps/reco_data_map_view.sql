-- @name: reco_data_map_view
-- @type: matview
-- @depends:
-- @auto_depends: true
-- @indexes:
--   - columns: ["month","year","reco_id","patient_id","form","reported_date"]
--     unique: true
--   - columns: ["id"]
--     method: btree
...
CREATE MATERIALIZED VIEW IF NOT EXISTS reco_data_map_view AS
    SELECT 
        CONCAT(a.month, '-', a.year, '-', a.reco_id, '-', a.patient_id, '-', a.form, '-', a.reported_date) AS id,
        a.month,
        a.year,
        a.reco_id,
        a.patient_id,
        ...
        MAX(NULLIF(a.geolocation->>'latitude','')::DOUBLE PRECISION) AS latitude,
        MAX(NULLIF(a.geolocation->>'longitude','')::DOUBLE PRECISION) AS longitude,
        MAX(NULLIF(a.geolocation->>'accuracy','')::DOUBLE PRECISION) AS accuracy,
        MAX(NULLIF(a.geolocation->>'altitude','')::DOUBLE PRECISION) AS altitude,
        MAX(NULLIF(a.geolocation->>'altitudeAccuracy','')::DOUBLE PRECISION) AS altitudeAccuracy,
        MAX(NULLIF(a.geolocation->>'heading','')::DOUBLE PRECISION) AS heading,
        MAX(NULLIF(a.geolocation->>'speed','')::DOUBLE PRECISION) AS speed,
    ...
    GROUP BY a.reco_id, a.month, a.year, a.patient_id, a.form, a.reported_date;
