...
# DatasetChart Model
# ----------------------------
class DatasetChart(db.Model, BaseModel, TenantMixin, TimestampMixin, SoftDeleteMixin, AuditMixin, StatusMixin):
    __tablename__ = "dataset_charts"
    __table_args__ = (
        db.UniqueConstraint("tenant_id", "name", name="uq_chart_name_per_tenant"),
        db.ForeignKeyConstraint(
            ["dataset_query_id", "dataset_id"],
            ["analy.dataset_queries.id", "analy.dataset_queries.dataset_id"],
            ondelete="CASCADE",
    ...
    dataset_query_id = db.Column(db.String(11), db.ForeignKey("analy.dataset_queries.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    # Relations
    dataset = db.relationship("Dataset", back_populates="dataset_charts", lazy="noload",foreign_keys=[dataset_id])
    dataset_query = db.relationship("DatasetQuery", back_populates="dataset_charts", lazy="noload",foreign_keys=[dataset_query_id])

    visualization_charts = db.relationship("VisualizationChart", back_populates="dataset_chart", cascade="all, delete-orphan")
    ...
    def validate_options(self):
        ...
        semantic:dict = self.options.get("semantic", {})
        visual:dict = self.options.get("visual", {})
        interaction:dict = self.options.get("interaction", {})
    ...
    def to_dict(self, include_relations=True):
        ...
        if include_relations:
            data.update({
                ...
                "query": self.dataset_query.to_dict(False) if self.dataset_query else None,
                "visualizations": [v.to_dict(False) for v in self.visualization_charts] if self.visualization_charts else []
            })
        return data
