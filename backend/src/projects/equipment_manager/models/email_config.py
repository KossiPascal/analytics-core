...
from backend.src.projects.equipment_manager.models.tickets import RepairTicket
...
class AlertRecipientConfig(db.Model, AuditMixin):
    ...
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    employee_id = db.Column(db.BigInteger,db.ForeignKey("eqpm.employees.id", ondelete="CASCADE"),nullable=True)
    position_id = db.Column(db.BigInteger,db.ForeignKey("eqpm.positions.id", ondelete="CASCADE"),nullable=True)
