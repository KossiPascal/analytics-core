...
from backend.src.app.routes.employees import get_allowed_employee_ids
from backend.src.projects.equipment_manager.models.email_config import AlertRecipientConfig
from backend.src.projects.equipment_manager.models.equipment import Equipment
...
from backend.src.projects.equipment_manager.models.tickets import DelayAlertRecipient, Issue, ProblemType, RepairTicket, TicketComment, TicketEvent
from backend.src.projects.equipment_manager.services.email_service import send_cc_failure_notification, send_ticket_notification
from backend.src.projects.equipment_manager.services.ticket_workflow import generate_ticket_number, get_next_stages, validate_transition
...
def list_tickets():
    ...
    is_admin   = bool(roles & {"admin", "superadmin"})
    ...
    if not is_admin:
        ...
        if caller_position_is_asc and caller_employee_id:
            ...
            if caller_orgunit_ids:
                from backend.src.app.models.user import UserOrgunitLink
                user_ids_in_zones = (
...
def create_ticket():
    ...
    if creator_dept_code:
        from backend.src.app.models.employee import Department as _Dept
...
def send_ticket(id):
    ...
    try:
        ...
        if failed_cc and user_id:
            from backend.src.app.models.user import User as AuthUser
...
def receive_from_repairer(id):
    ...
    if not is_admin:
        ...
        authorized = False
        if position_id:
            from backend.src.app.models.employee import Position
