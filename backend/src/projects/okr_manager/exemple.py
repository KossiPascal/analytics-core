...
# # -------------------------------
# okr_global = ORCGlobal(title="Croissance Santé 2026", start_date=date(2026,1,1), end_date=date(2026,12,31))
# session.add(okr_global)
# session.commit()

# okr_team_sales = ORCTeam(team_id=team_sales.id, okr_global_id=okr_global.id)
# okr_team_tech = ORCTeam(team_id=team_tech.id, okr_global_id=okr_global.id)
# session.add_all([okr_team_sales, okr_team_tech])
# session.commit()

# initiative_vaccine = Initiative(title="Augmenter couverture vaccinale", okr_team_id=okr_team_tech.id)
...
# # Lien activité → KR
# link1 = ActivityLinkKR(activity_id=activity1.id, kr_id=kr_vaccine.id, impact=0.8)
# session.add(link1)
# session.commit()
...
# # -------------------------------
# event1 = KREvent(kr_id=kr_vaccine.id, value=1200, date=date(2026,3,1), source="activity")
# session.add(event1)
# session.commit()
...
# okr_team_progress = calculate_okr_team_progress(session, okr_team_tech)
# okr_global_progress = calculate_okr_global_progress(session, okr_global)
# project_progress = calculate_project_progress(session, project_vaccine)
# snapshot = update_snapshot(session, okr_global.id)
...
# print(f"ORC Team Progression: {okr_team_progress:.2f}%")
# print(f"ORC Global Progression: {okr_global_progress:.2f}%")
