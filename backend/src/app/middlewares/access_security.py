    ...
    def decorator(func):
        ...
        def wrapped(*args, **kwargs):
            ...
            if roles_names or permissions_names: 
                # permissions_names
                mainRoles:list[Role] = user.roles or []

                if roles_names:
                    roles:set[str] = set([r.name for r in mainRoles if r.name])
                ...
                if permissions_names:
                    ...
                    for rp in mainRoles:
                        permissions:list[Permission] = rp.permissions
                        for p in permissions:
                            mainPermissions.append(p.name)

                    permissions:set[str] = set(mainPermissions)
                    required_permissions = [perm for perm in permissions_names]

                    if permissions.isdisjoint(required_permissions):
                        raise Forbidden("[REQUIRE_AUTH] Insufficient permissions")
            ...
            # g.current_permissions = set(payload.get("permissions", []))
