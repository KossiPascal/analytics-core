...
from backend.src.app.models.role_permission import Permission, Role
...
from backend.src.app.models._controls import MetaxMixin, TimestampMixin
...
class User(db.Model, MetaxMixin):
    ...
    employe = db.relationship("Employee", back_populates="user",lazy="noload")
    ...
    roles = db.relationship("Role",secondary="users_roles",lazy="noload",backref=db.backref("users", lazy="noload"))
    histories = db.relationship("DataSourceHistory",lazy="noload",cascade="all, delete-orphan",foreign_keys="DataSourceHistory.user_id")
    memberships = db.relationship("Membership", back_populates="user", lazy="noload", cascade="all, delete-orphan")
    okr_activity_owners = db.relationship("ActivityOwner", back_populates="user", lazy="noload", cascade="all, delete-orphan", foreign_keys="[OkrActivityOwner.user_id]")
    okr_project_tasks = db.relationship("ProjectTask", back_populates="assigned_to", lazy="noload", cascade="all, delete-orphan",foreign_keys="[OkrProjectTask.assigned_to_id]")
...
class Membership(db.Model, TimestampMixin):
    ...
    scope_id = db.Column(db.BigInteger)

    tenant = db.relationship("Tenant", back_populates="memberships", lazy="noload", foreign_keys=[tenant_id])
...
class Membership(db.Model, TimestampMixin):
    ...
    # 🔥 scopes explicites (clean)
    global_scope = db.Boolean(default=False)
    region_id = db.Column(db.BigInteger, db.ForeignKey("regions.id"), nullable=True)
