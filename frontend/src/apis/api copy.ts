...
//   const tokenExp = tokenProvider.getExpiration(); // timestamp Unix en secondes
...
// // FETCH WITH AUTH
// // ------------------------
// async function fetchWithAuth<T>(url: string, options: RequestInit = {}, retry = true): Promise<T> {
//   // const token = tokenProvider.get();
//   const token = await getValidToken();
...
//       // Token expiré côté backend → logout
//         tokenProvider.clear();
//         window.location.href = "/auth/login";
//         throw new Error("Token expiré ou invalide");
//       // return withTokenRefresh(token =>
//       //   fetchWithAuth(url, { ...options, headers: { ...(options.headers || {}), Authorization: `Bearer ${token}` } }, false)
//       // );
//     }
