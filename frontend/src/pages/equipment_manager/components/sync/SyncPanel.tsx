...
export function SyncPanel({ onSyncComplete }: Props) {
  ...
  const [syncingAscs, setSyncingAscs] = useState(false);
  ...
  const handleSyncAscs = async () => {
    setSyncingAscs(true);
    try {
      const res = await syncApi.syncAscs();
      if (res.success && res.data) {
        const d = res.data;
        if (d.error) {
          toast.error(d.error);
  ...
  return (
    <div className={shared.headerActions}>
      ...
      <Button variant="outline" size="sm" onClick={handleSyncAscs} isLoading={syncingAscs} leftIcon={<RefreshCw size={14} />}>
        Sync ASCs
      </Button>
    </div>
  );
