...
const convertToConnParams = (form: DataSourceConnection): DataSourceConnectionParams => {
  return {
    id: form.id,
    type: form.type,
    name: form.name,
...
export const DatabaseConnectionTab: React.FC<{ showTitle?: boolean, afterUpsert?: () => Promise<void>, onlyCardField?: boolean }> = ({ showTitle = true, onlyCardField = false, afterUpsert }) => {
  const [form, setForm] = useState<DataSourceConnection>(DEFAULT_FORM);
  ...
  const fetchDataSourceConnections = async () => {
    setLoading(true);
    try {
      // const response = await API.list<DataSourceConnection>();
      // if (response?.status === 200) {
      //   setDataSourceConnections(response.data || []);
      // } else {
  ...
  const updateField = useCallback((key: keyof DataSourceConnection, value: any) => {
    setForm(prev => ({ ...prev, [key]: value }));
  }, []);

  const edit = (conn: DataSourceConnectionParams) => {
  };
  ...
  const handleEdit = (conn: DataSourceConnection) => {
    setEditing(true);
  ...
  const handleDeleteClick = (org: DataSourceConnection) => {
    setForm(org);
  ...
  const renderConnexionHeader = () => (
    <div className={styles.cardHeader}>
      ...
      <div className={styles.buttonGroup}>
        <button className={`${styles.btn} ${styles.btnOutline} ${styles.btnSmall}`} onClick={fetchDataSourceConnections} disabled={loading}>
  ...
  const columns: Column<DataSourceConnection>[] = [
    {
      key: 'id',
      header: 'ID',
      sortable: true,
    ...
    <Card>
      <div className={styles.card}>
        ...
        {loading ? (<div className={styles.loading}><RefreshCw size={24} className="animate-spin" /></div>) :
          dataSourceConnections.length === 0 ? (renderEmptyState()) :
            (
              <Table
                data={dataSourceConnections as any}
