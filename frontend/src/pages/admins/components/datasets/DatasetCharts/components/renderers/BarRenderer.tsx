...
export const BarRenderer = ({ chart, query, data }: ChartRenderProp) => {
  ...
  const barRadius = options.bar_radius ?? [4, 4, 0, 0];
  const barGap = options.bar_gap ?? 4;
  const barCategoryGap = options.bar_category_gap ?? "20%";
  ...
  const chartData = useMemo(() => {
    return cleanRows.map((row: any) => {
      ...
      metricColumns.forEach((col) => {
        const v = row[col];
        r[col] = Number.isFinite(Number(v)) ? Number(v) : 0;
      });
  ...
  return (
    <div style={{ width, height }}>
      {options.title && (
        <div className="text-lg font-semibold mb-1 text-center">{options.title}</div>
      )}
      {options.subtitle && (
        <div className="text-gray-500 text-sm mb-3 text-center">{options.subtitle}</div>
      )}
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          {isHorizontal ? (
            <>
              <XAxis type="number" />
              <YAxis type="category" dataKey={xKey} width={options.y_label_width ?? 150} />
            </>
          ) : (
            <>
              <XAxis
              ...
              <YAxis
                yAxisId="left"
                tickFormatter={(v) =>
                  options.y_axis_format === "percent" ? `${(v * 100).toFixed(2)}%` : v
                }
              />
          ...
          {metricColumns.map((col: string, i: number) => (
            <Bar
              ...
              animationBegin={(options.animation_delay_per_bar ?? 0) * i}
            >
              {options.show_labels && (
                <LabelList
                  dataKey={col}
                  ...
                  formatter={(v:any) => {
                    if (options.label_format === "percent") return (v * 100).toFixed(1) + "%";
                    return v;
                  }}
                  angle={options.label_angle ?? 0}
                  offset={options.label_offset ?? 0}
                />
              )}
            </Bar>
          ))}
