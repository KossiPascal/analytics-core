...
import { ChwsRecoReport, FamilyPlanningReport, HouseholdRecapReport, MorbidityReport, PcimneNewbornReport, PromotionReport, getChwsRecoReportRepository, getFamilyPlanningReportRepository, getHouseholdRecapReportRepository, getMorbidityReportRepository, getPcimneNewbornReportRepository, getPromotionReportRepository, getRecoMegSituationReportRepository } from '../../entities/Reports';
...
export async function VALIDATE_RECO_MEG_REPORTS(req: Request, res: Response, next: NextFunction) {
    try {
        var { userId, months, year, recos } = req.body;
        if (1 == 1) {
            if (months && year && recos) {
