...
import { ChwsRecoReport, FamilyPlanningReport, getChwsRecoReportRepository, getFamilyPlanningReportRepository, getHouseholdRecapReportRepository, getMorbidityReportRepository, getPcimneNewbornReportRepository, getPromotionReportRepository, getRecoMegSituationReportRepository, HouseholdRecapReport, MorbidityReport, PcimneNewbornReport, PromotionReport, RecoMegSituationReport } from "../entities/Reports";
...
export async function SEND_PROMOTONAL_ACTIVITIES_TO_DHIS2(req: Request, res: Response, next: NextFunction) {
    ...
    dataToSend.orgunit = 'erdjSX8MtGO';
        ...
        try {
            ...
            if (error.response) {
                ...
                if (error.response.status === 401) {
                    return dhis2.res.status(201).json({ status: 201, data: `ERROR: Authentication échoué, problème avec vos identifiants DHIS2: ${error.response}` });
